...
function checkRequiredDirectories() {
  console.log(`${consoleStyles.info}Checking required directories...${consoleStyles.reset}\n`);
  ...
  if (createdDirs.length > 0) {
    console.log(
      `\n${consoleStyles.warning}Note: Please add required files to: ${createdDirs.join(', ')}${consoleStyles.reset}`
    );
  }
  ...
  const hasImages = hasFilesRecursive(imagesDir, ['.jpg', '.jpeg', '.png', '.gif', '.webp']);
  const hasTextFiles = hasFilesRecursive(textDir, ['.xlsx', '.xls']);

  if (!hasImages || !hasTextFiles) {
    console.log('\nMissing required files:');
    if (!hasImages) logWarning('No images found in images directory or its subfolders');
    if (!hasTextFiles) logWarning('No Excel files found in text directory or its subfolders');
...
function getAllImages(dir = imagesDir) {
  if (!fs.existsSync(dir)) {
    console.error('Images directory does not exist. Please create it and add images.');
    return [];
  }
...
async function drawEdgePoints(imageBuffer, edgePoints, color = 'red') {
  try {
    ...
    const svgElements = edgePoints.map((point) => `<circle cx="${point.x}" cy="${point.y}" r="1" fill="${color}"/>`);

    const svgBuffer = createSVGBuffer(metadata.width, metadata.height, svgElements);
                    ...
                    switch (obj.type) {
                      ...
                      case 'text':
                        return `<text 
                                ...
                                dominant-baseline="${obj.dominantBaseline || 'auto'}"
...
async function findEmptyRegions(imageBuffer, minWidth = 100, minHeight = 30) {
  try {
    ...
    for (const region of regions) {
      ...
      if (region.y - (currentRegion.y + currentRegion.height) < cellSize * 4) {
...
function getFormatProperties(position, colorSegments, boldSegments) {
  ...
  const colorSegment = colorSegments.find((seg) => position >= seg.start && position < seg.start + seg.length);
  if (colorSegment) {
    format.color = colorSegment.color;
  }

  // Check bold segments
  const boldSegment = boldSegments.find((seg) => position >= seg.start && position < seg.start + seg.length);
  if (boldSegment) {
    format.isBold = true;
  }
...
function wrapText(text, maxWidth, fontSize = 30, x = 0, y = 0, colorSegments = [], boldSegments = []) {
  if (!text) return [];
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    const format = getFormatProperties(wordStart, colorSegments, boldSegments);

    // Calculate width based on bold status
    const wordWidth = calculateTextWidth(word, format.isBold ? fontSize * 1.1 : fontSize);
    const spaceWidth = i > 0 ? calculateTextWidth(' ', fontSize) : 0;

    if (currentLine.width + wordWidth + spaceWidth > maxWidth && currentLine.segments.length > 0) {
...
async function drawRegionsWithText(imageBuffer, regions, textObjects) {
  try {
    ...
    if (Math.min(metadata.width, metadata.height) < UPSCALE_THRESHOLD) {
      upscaleFactor = Math.min(MAX_UPSCALE, UPSCALE_THRESHOLD / Math.min(metadata.width, metadata.height));
    }
          ...
          >
            ${obj.colorSegments
              .map((segment, index) => {
                ...
                return `<tspan 
                fill="${segment.color}"
                ${index > 0 ? `dx="${calculateTextWidth(' ', obj.fontSize)}"` : ''}
                shape-rendering="geometricPrecision"
                text-rendering="geometricPrecision"
              >${escapedText}</tspan>`;
              })
...
async function getAllTextFiles(dir = textDir) {
  try {
    if (!fs.existsSync(dir)) {
      fs.mkdirSync(dir);
      console.log('Created text directory. Please add Excel files and run again.');
      return [];
    }
      ...
      if (stat.isDirectory()) {
        ...
        if (ext === '.xlsx' || ext === '.xls') {
          try {
            ...
            console.warn(`⚠️ Skipped Excel file ${item}: ${excelError.message}`);
          }
        }
      }
    }
...
function getThemeColor(workbook, theme) {
  ...
  try {
    if (workbook.theme && workbook.theme.themeElements && workbook.theme.themeElements.clrScheme) {
...
async function processImage(imagePath) {
  try {
    ...
    const relativePath = path.relative(imagesDir, path.dirname(path.join(imagesDir, imagePath)));
    ...
    const textFiles = fs.existsSync(textSubDir) ? await getAllTextFiles(textSubDir) : await getAllTextFiles(textDir); // Fallback to root text dir if subfolder doesn't exist
    ...
    const [metadata, emptyRegions] = await Promise.all([sharp(imageBuffer).metadata(), findEmptyRegions(imageBuffer)]);
    ...
    const processPromises = textFiles.map(async (textContent, i) => {
      ...
      try {
        const result = await insertTextIntoRegions(imageBuffer, textContent, emptyRegions);
...
async function processBatch(images, batchSize = 4) {
  ...
  for (const batch of batches) {
    const results = await Promise.all(batch.map((image) => processImage(image)));
  ...
  if (images.length > 0) {
    console.log(
      `\n✨ Complete: ${successful}/${images.length} successful (${((successful / images.length) * 100).toFixed(1)}%)`
    );
  }
}
...
async function calculateOptimalFontSize(imageBuffer, region, text, paragraphs) {
  try {
    ...
    while (minSize <= maxSize && iterations < maxIterations) {
      ...
      paragraphs.forEach((para, index) => {
        ...
        for (const char of para.text) {
          const isBold = para.boldSegments.some((seg) => position >= seg.start && position < seg.start + seg.length);
          const charWidth = calculateTextWidth(char, isBold ? boldFontSize : fontSize);
      ...
      if (!fitsFailed && totalHeight <= effectiveHeight && testLines.length > 0) {
...
function optimizeLayout(wrappedLines, region, lineHeight, fontSize) {
  ...
  const startY = region.y + Math.max(adjustedLineHeight * 0.2, (region.height - finalContentHeight) / 2);
...
function determineVerticalAlignment(region, totalTextHeight, textLines) {
  ...
  const contentDensity =
    textLines.reduce((sum, line) => sum + (line.text ? line.text.length : 0), 0) / textLines.length;
  ...
  const hasLongLines = textLines.some((line) => line.text && line.text.length > 50);
...
async function insertTextIntoRegions(imageBuffer, textContent, emptyRegions) {
  try {
    ...
    const regions = emptyRegions.filter((r) => r.width >= 100 && r.height >= 30);
    if (regions.length === 0) {
    ...
    const paragraphs = text.split('\n').map((para, index) => {
      ...
      const paraColorSegments = textContent.colorSegments
        .filter((seg) => seg.start >= paraStart && seg.start < paraStart + para.length)
        .map((seg) => ({
          ...seg,
    ...
    const { normalFontSize, boldFontSize, estimatedLines } = await calculateOptimalFontSize(
      imageBuffer,
      region,
      text,
      paragraphs
    ...
    paragraphs.forEach((paragraph, pIndex) => {
      ...
      words.forEach((word, wIndex) => {
        ...
        const isBold = paragraph.boldSegments.some(
          (seg) => seg.start <= wordStart && seg.start + seg.length >= wordStart + word.length
        );
        ...
        const wordWidth = calculateTextWidth(word, isBold ? boldFontSize : normalFontSize);
          ...
          if (currentLine) {
            wrappedLines.push({
              ...
              isBold: paragraph.boldSegments.some(
                (seg) =>
                  seg.start <= currentLineStart && seg.start + seg.length >= currentLineStart + currentLine.length
              ),
      ...
      if (currentLine) {
        wrappedLines.push({
          ...
          isNewParagraph: false,
          isBold: paragraph.boldSegments.some(
            (seg) => seg.start <= currentLineStart && seg.start + seg.length >= currentLineStart + currentLine.length
          ),
          colorSegments: currentLineColors,
        });
      }
    ...
    const paragraphSpacing = lineHeight * (wrappedLines.length > 10 ? 1.1 : 1.3);

    // Calculate total text height
    const totalTextHeight = wrappedLines.reduce((height, line) => {
      if (line.isParagraphSpace) {
        return height + (wrappedLines.length > 10 ? lineHeight * 0.5 : paragraphSpacing);
      }
      return height + lineHeight;
    }, 0);

    // Determine vertical alignment
    const verticalAlignment = determineVerticalAlignment(region, totalTextHeight, wrappedLines);
    ...
    startY = Math.max(
      region.y + topPadding,
      Math.min(startY, region.y + region.height - totalTextHeight - bottomPadding)
    );
...
function createProgressBar(total, title = 'Progress') {
  ...
  function complete() {
    ...
    process.stdout.write(
      `${consoleStyles.info}${title}: ${consoleStyles.reset}` +
        `[${consoleStyles.success}${'█'.repeat(barWidth)}${consoleStyles.reset}] ` +
        `100.0% (${total}/${total})\n`
    );
  }
